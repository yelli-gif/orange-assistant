...
function App() {
  ...
  return (
    <div className="mobile-frame">
      ...
      <main className="content-area no-scrollbar min-h-0">
      ...
      {showNav && (
        <button 
          ...
          className="absolute bottom-24 right-6 w-14 h-14 bg-black text-white rounded-2xl flex items-center justify-center shadow-2xl z-40 border border-white/10 active:scale-95 transition-all"
        >
          <Bot size={28} />
        </button>
      )}

      {/* Navigation Basse - Uniquement si showNav est vrai */}
      {showNav && (
        <nav className="bottom-nav">
          <button onClick={() => setScreen('DASHBOARD')} className={`flex flex-col items-center gap-1 flex-1 transition-all ${screen === 'DASHBOARD' ? 'text-[#FF7900]' : 'text-slate-400'}`}>
            <div className={`p-4 rounded-[1.2rem] transition-all ${screen === 'DASHBOARD' ? 'bg-[#FFF5ED]' : ''}`}>
              <Home size={26} strokeWidth={screen === 'DASHBOARD' ? 3 : 1.5} />
            </div>
